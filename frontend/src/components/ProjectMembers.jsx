...
export default function ProjectMembers({ projectId, onClose }) {
  ...
  const loadMembers = async () => {
    try {
      setLoading(true);
      const data = await membersApi.getProjectMembers(projectId);
      setMembers(data);
    } catch (err) {
  ...
  const handleAddMember = async () => {
    ...
    try {
      await membersApi.addProjectMember(projectId, parseInt(selectedUserId), selectedRole);
      await loadMembers();
  ...
  const handleRemoveMember = async (userId) => {
    ...
    try {
      await membersApi.removeProjectMember(projectId, userId);
      await loadMembers();
    } catch (err) {
  ...
  const handleUpdateRole = async (userId, newRole) => {
    try {
      await membersApi.updateProjectMemberRole(projectId, userId, newRole);
      await loadMembers();
    } catch (err) {
